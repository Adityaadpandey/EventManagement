...
export const createDiscountSchema = z
  ...
  .refine(
    (data) => {
      if (data.discountType === "PERCENTAGE" && !data.discountPct) {
        return false;
      }
      if (data.discountType === "FLAT" && !data.discountAmt) {
        return false;
      }
      return true;
    },
    {
      message:
        "discountPct is required for PERCENTAGE type, discountAmt is required for FLAT type",
      path: ["discountType"],
    },
  )
